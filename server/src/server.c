...
int main() {
  ...
  rtcSetDataChannelCallback(pc, on_data_channel_callback) ;


  // Answer für den Server erstellen; Der Server liefert eine Antwort
  // der Client erstellt ein OFFER -> libdatachannel soll selbst aushandeln,
  // was gewählt wird

  
  // Der Server bekommt an dieser Stelle das SDP des Clients
  
  printf("Offer vom Client wird gelesen...\n");
  getchar();

  // Client SDP lesen
  FILE* offer_fptr = fopen("/home/josh/development/temp/offer_from_client.txt", "r");
  if(offer_fptr == NULL) {
    printf("Fehler beim lesen Datei.\n");
    exit(EXIT_FAILURE);
  }

  char client_offer[1024];
  int read = 0;
  if ( (read = fread(client_offer, sizeof(char), 1024, offer_fptr)) == 0) {
    printf("Client Offer konnte nicht gelesen werden!\n.");
  };

  printf("Client Offer wurde erfolgreich mit %d Bytes eingelesen.\n", read);

  rtcSetRemoteDescription(pc, client_offer, "offer");

  char answer[1024];
  int answer_size = rtcCreateAnswer(pc, answer, 8192);

  // Answer muss zurück an den Client geschickt werden
  FILE* answer_fptr = fopen("/home/josh/development/temp/answer_from_server.txt", "w");
  if(answer_fptr == NULL) {
    printf("Fehler beim erstellen der Datei.\n");
    exit(EXIT_FAILURE);
  }
  
  fprintf(answer_fptr, "%s", answer);
  
  fclose(answer_fptr);
  printf("Antwort erfolgreich in Datei geschrieben!\n");

  while(running) {
    sleep(1);
    if(data_channel_id != 0) {
      char message_buffer[512];
      int buffer_size = sizeof(message_buffer);
      int ret = rtcReceiveMessage(data_channel_id, message_buffer, &buffer_size);
